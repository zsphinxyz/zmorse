  ...
  return (
    <div className="flex flex-col mt-5" >
      ...
      <div className="ring-1 ring-white/20 bg-white/5 group " style={{ display: hint ? 'flex' : 'none', flexDirection: reverse ? 'column-reverse' : 'column' }}>
        <Image draggable={false} src={`/imgs/${letter_imgs[rand].img}`} width={500} height={500} alt={'hint image'} className={`${reverse ? `group-hover:opacity-100 opacity-0 [animation-play-state:paused] group-hover:[animation-play-state:running] ` : 'peer'} overflow-hidden size-[150px] object-cover select-none transition delay-100`}
          style={{
