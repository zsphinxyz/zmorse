  ...
  return (
    <div className="flex flex-col mt-5" >
      ...
      <div className="ring-1 ring-white/20 bg-white/5 group " style={{ display: hint ? 'flex' : 'none', flexDirection: reverse ? 'column-reverse' : 'column' }}>
        <Image draggable={false} src={`/imgs/${img.img}`} width={500} height={500} alt={'hint image'} className={`${reverse ? `group-hover:opacity-100 group-hover:ani-sprite opacity-0 [animation-play-state:paused] group-hover:[animation-play-state:running] ` : 'peer'} overflow-hidden size-[150px] object-cover select-none transition delay-100`}
          style={{objectPosition: `-${(img.steps - 1) * 150}px`}}
        />
        <p className={`text-white/50 text-lg text-center font-medium ${reverse ? '' : 'group-hover:opacity-100 opacity-0' } transition delay-100 block`}>{img.img.replace('.png', '')}</p>
      </div>
