...
export const metadata:Metadata = {
  ...
  keywords: ["Morse Code Guesser", "guessMorse", "guess Morse Code", "Morse Code Guessing", "Morse Code Guess", "Morse Code Guessr"]
};

const ParamEnum = ["morse", "letter", "audio"] as const;

export default async function GuessrPage({searchParams}: {searchParams: Promise<{q: typeof ParamEnum[number] | undefined}>}) {
  "use server";
  
  let {q} = await searchParams;
  if(!q || !(ParamEnum.includes(q))) q = "morse";
  
  const PARAMS: Array<{id:number, param: typeof ParamEnum[number], component: ReactNode}> = [
    {
      id: 0,
      param: "morse",
      ...
      <div className="flex my-2">
        {
          PARAMS.map( p => (
            <form key={p.id} action={handleForm.bind(null, p.param)}>
              <button data-active={p.param===q} className="bg-slate-800 data-[active=true]:bg-green-700 border border-slate-800 data-[active=true]:border-white/50 px-6 py-2 capitalize data-[active=false]:hover:border-white/50" type="submit">
                {p.param}
              </button>
            </form>
          ))
