...
export default function Page() {
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      if (e.code !== key || e.repeat || document.activeElement === mappedKey.current) return;
      handleDown();
    }
  ...
  function handleKeyInput(e: React.KeyboardEvent<HTMLInputElement>) {
    ...
    if(mappedKey.current) {
      mappedKey.current!.style.scale = "0.9";
      setTimeout(() => {
        mappedKey.current!.style.scale = "1";
      }, 250)
    }
    setKey(e.code);
  }
  ...
  function clearAll() {
    morseStringRef.current!.innerText = "";
    convertRef.current!.innerText = "";
  }
    ...
    <section className="flex h-full gap-2 flex-col relative bg-[#1e3f44]">
      <section className="p-2 flex flex-col h-full gap-2 w-full max-w-3xl mx-auto">
        <div className="w-full  h-full gap-2 flex flex-col *:flex-1 *:min-h-20 border border-white/30 p-2 rounded-lg" >
          <p className="p-2 rounded-md font-mono bg-white/5 tracking-[-2] [word-spacing:5px]" ref={morseStringRef} />
          <p className="p-2 rounded-md tracking-wider bg-white/5" ref={convertRef} />
        </div>

        <div className="flex flex-col gap-2 bg-black/15 rounded-md p-2 w-full mt-auto">
          <div className="w-full flex justify-between items-center">
            <div title="Keyboard Key" className="bg-white/10 rounded-sm shadow-black/50 text-sm shadow-sm">
              <input type="text" id="key" readOnly ref={mappedKey} onKeyDown={handleKeyInput} value={key} className="bg-transparent min-w-16 text-center [field-sizing:content] px-2 focus:outline-green-500"/>
            </div>
            <div title="Display" ref={morseRef} className="bg-white/10 text-white px-1 leading-[1] w-16 h-full text-center rounded-sm overflow-hidden text-nowrap shadow-sm shadow-black/50" />
            <div title="Clear" role="button" onClick={clearAll} className="bg-white/10 select-none cursor-pointer shadow-black/50 shadow-sm px-2 text-sm rounded-sm active:scale-95">Clear</div>
          </div>
          ...
          <div className="w-full">
            <div
              ...
              className="bg-gradient-radial relative select-none text-center from-slate-800 to-slate-900 rounded-lg py-20 w-full max-w-80 mx-auto touch-none border-2 border-neutral-400 [box-shadow:4px_6px_2px_#25252566] active:[box-shadow:1px_1px_0_#25252566] active:scale-[.99] transition-all duration-[25ms]"
            >
              <div ref={lightRef} data-on="null" className="size-2 rounded-full data-[on=null]:bg-black bg-gray-300 data-[on=true]:bg-red-500 data-[on=true]:[box-shadow:0_0_20px_red] absolute top-3 right-3" />
